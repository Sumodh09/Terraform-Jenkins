...
resource "aws_s3_object" "my_script_zip" {
  ...
  key    = "CSV_Script.zip" 
  source = "/var/lib/jenkins/workspace/AWS-Config-rule/deployment/Child-Script/CSV_Script.zip"
  acl    = "private"
...
resource "aws_s3_object" "my_script_zip_name" {
  ...
  source = "/var/lib/jenkins/workspace/AWS-Config-rule/deployment/Child-Script/Rule_Name_Check_Script.zip"
  acl    = "private"
